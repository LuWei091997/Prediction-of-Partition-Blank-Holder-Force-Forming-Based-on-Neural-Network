...
if __name__ == '__main__':
    ...
    model = load_model('../output/model/Dense_model.h5')
    pred = model.predict(X_test)
    print(pred.shape)
    y_test = y_test.reshape(len(y_test), 2500)
    pred = pred.reshape(len(pred), 2500)

    save_data.save_data('../output/pred_result/pred_dense.csv', pred)
    save_data.save_data('../output/pred_result/origin_dense.csv', y_test)
